...
export interface UseRefreshControlReturn
  extends Pick<
      UseRefreshStateReturn,
      'refreshRate' | 'setRefreshRate' | 'cycleRefreshRate' | 'config'
    >,
...
export const useRefreshControl = (
  initialRate: RefreshRate = 'disabled'
): UseRefreshControlReturn => {
  const { refreshRate, setRefreshRate, cycleRefreshRate, config } = useRefreshState(initialRate)

  const { refreshInterval, staleTime, queryKeySuffix, isActive } = useRefreshInterval(refreshRate)
